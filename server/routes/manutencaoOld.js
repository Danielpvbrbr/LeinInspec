...
exports.manute_register = async (req, res) => {
    const { descricao, oficina, garantiaDate, dataCreate, veiculo, placa } = req.body;
    ...
    try {
        const [result] = await db.query(
            'INSERT INTO manutencao (descricao, oficina, garantiaDate, dataCreate, veiculo, placa) VALUES (?,?,?,?,?,?)',
            [descricao, oficina, garantiaDate, dataCreate, veiculo, placa]
        );
...
exports.manute_atualizar = async (req, res) => {
    const { descricao, oficina, garantiaDate, veiculo, placa, id } = req.body;
    ...
    try {
        const [result] = await db.query('UPDATE manutencao SET descricao=?, oficina=?, garantiaDate=?, veiculo=?, placa=? WHERE id=?', [descricao, oficina, garantiaDate, veiculo, placa, id]);
